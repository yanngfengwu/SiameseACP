...
def draw_importance(importance_dict, name):
    ...
    plt.savefig('/mnt/sdb/home/yxt/CBC_and_DATA/tmpPics/{} importance distribution.png'.format(name))
    plt.show()
...
def change_and_test(x, y, model, name):
    ...
    return importance
def change_and_test2(x, y, model, name):
    # 替换数据并进行测试
    # 初始化替换需要用到的数据
    new_x = torch.tensor(x)
    a2n_dict = {'A': 1, 'R': 2, 'N': 3, 'D': 4, 'C': 5, 'Q': 6, 'E': 7, 'G': 8, 'H': 9, 'I': 10,
...
def MyPlot(dict_pos, dict_neg):
    ...
    fig.tight_layout()
    plt.savefig('/mnt/sdb/home/yxt/CBC_and_DATA/tmpPics/pred2_statistics.png')
if __name__ == '__main__':
    ...
    LEN = 50
    pos_cnt0, neg_cnt0, pos_cnt, neg_cnt = dict(), dict(), dict(), dict()
    ...
    for i in range(1, LEN + 1):
        pos_cnt[i] = pos_cnt0[i] = 0
        neg_cnt[i] = neg_cnt0[i] = 0
    ...
    #             new_pos_scores = change_and_test(x, y, model, name='non-ACP_len={}_no.={}_'.format(LEN, i))
    ...
    #         outputs = model.trainModel(x)[0].tolist()
    #         if outputs[0] < outputs[1]:
    #             new_neg_scores = change_and_test(x, y, model, name='ACP_len={}_no.={}_'.format(LEN, i))
    #             for idx in neg_scores.keys():
    #                 for amino_acid in amino_acids:
    ...
    for i in tqdm(range(388)):
        ...
        if y == 0:  # and getLen(x[0].tolist()) == LEN:
            outputs = model.trainModel(x)[0].tolist()
            if outputs[0] > outputs[1]:
                new_pos_scores = change_and_test2(x, y, model, name='non-ACP_len={}_no.={}_'.format(LEN, i))
                for idx in pos_scores.keys():
                    for amino_acid in amino_acids:
                        pos_scores[idx][amino_acid] += new_pos_scores[idx][amino_acid]
                pos_cnt0[getLen(x[0].tolist())] += 1
        if y == 1:  # and getLen(x[0].tolist()) == LEN:
            outputs = model.trainModel(x)[0].tolist()
            if outputs[0] < outputs[1]:
                new_neg_scores = change_and_test2(x, y, model, name='ACP_len={}_no.={}_'.format(LEN, i))
                for idx in neg_scores.keys():
                    for amino_acid in amino_acids:
                        neg_scores[idx][amino_acid] += new_neg_scores[idx][amino_acid]
                neg_cnt0[getLen(x[0].tolist())] += 1
    for i in range(1, LEN + 1):
        for j in range(i, LEN + 1):
            if j in pos_cnt0:
                pos_cnt[i] += pos_cnt0[j]
            if j in neg_cnt0:
                neg_cnt[i] += neg_cnt0[j]
    for i in range(LEN):
        for amino_acid in amino_acids:
            pos_scores[i][amino_acid] /= pos_cnt[i + 1]
            neg_scores[i][amino_acid] /= neg_cnt[i + 1]
    draw_importance(pos_scores, 'ACP2_len={}_all'.format(LEN))
    draw_importance(neg_scores, 'non-ACP2_len={}_all'.format(LEN))
    ...
    MyPlot(neg_acid_cnt, pos_acid_cnt)
