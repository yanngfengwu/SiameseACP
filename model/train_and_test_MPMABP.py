...
from preprocess.get_data import collate, collate_5, get_prelabel, MyDataSet
...
from MyUtils.util_cal import caculate_metric, caculate_metric_5
from model.ContrastModel import newModel2, ContrastiveLoss
...
data, label, seq = get_original_data_MPMABP('../dataset/MPMABP')# 这边要注意最好从0开始，不然后面五分类莫名其妙报错，loss不能回归
# 不能直接取前多少位或者后多少位，它不均匀
# train_data, train_label, train_seq = data[:4892, :], label[:4892], seq[:4892]# 6115*0.88
# test_data, test_label, test_seq = data[4892:, :], label[4892:], seq[4892:]
ran1 = random.sample(range(0, 6115), 4892)
ran2 = [x for x in range(0, 6115)if x not in ran1 ]
# print(ran1, ran2)
train_data, train_label = data[ran1, :], label[ran1]# 6115*0.88
train_seq = list(np.array(seq)[ran1])
test_data, test_label = data[ran2, :], label[ran2]
test_seq = list(np.array(seq)[ran2])
...
train_iter_cont = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                              shuffle=True, collate_fn=collate_5)
...
#                         vec = torch.cat((vec, torch.tensor(seq2vec[z[i]]).to(device)), dim=0)
            ...
            metric1, roc_data1, prc_data1 = caculate_metric_5(pre1, real1, pred_pro1)
