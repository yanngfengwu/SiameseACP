...
for num_model in range(10):
    ...
    for epoch in range(EPOCH):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            if best_acc > 0.94:
                torch.save(net, '/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Alter.pkl'.format(best_acc))     # 保存整个神经网络的模型结构以及参数
                # torch.save(net.state_dict(), '/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Alter_params.pkl'.format(best_acc))  # 只保存模型参数
                torch.save(metric1,  '/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Alter_metrics.pt'.format(best_acc))
                np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Alter_roc.txt'.format(best_acc), roc_data1[:-1], delimiter=' ', fmt = '%s')
                np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Alter_roc2.txt'.format(best_acc), roc_data1, delimiter=' ', fmt='%s')
                np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Alter_prc.txt'.format(best_acc), prc_data1[:-1], delimiter=' ', fmt = '%s')
                np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Alter_prc2.txt'.format(best_acc), prc_data1, delimiter=' ', fmt='%s')
                print('roc_data:\n', roc_data1)
                print('\nprc_data\n', prc_data1)
...
'''
nohup python Save_A.py > /mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/Save_A_lr=1e-4_epoch=300.log 2>&1 &
nohup python Save_A.py > /mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/Save_A_lr=1e-5_epoch=3000.log 2>&1 &
'''