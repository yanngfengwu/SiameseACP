...
config.lr = 1e-5
config.epoch = 3000
train_data, train_label, train_seq = get_original_data_Anti_new("../dataset/AntiACP2.0_Main/internal")
...
#             print(1)
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            if best_acc > 0.76:
                torch.save(net, '/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Main_preNew.pkl'.format(best_acc))
                # torch.save(metric1,  '/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Main_preNew_metrics.pt'.format(best_acc))
                np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Main_preNew_roc.txt'.format(best_acc), roc_data1[:-1], delimiter=' ', fmt = '%s')
                np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Main_preNew_roc2.txt'.format(best_acc), roc_data1, delimiter=' ', fmt='%s')
                np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Main_preNew_prc.txt'.format(best_acc), prc_data1[:-1], delimiter=' ', fmt = '%s')
                np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_model_Main_preNew_prc2.txt'.format(best_acc), prc_data1, delimiter=' ', fmt='%s')

            print(f"best_acc: {best_acc},metric:{metric1}")

'''
nohup python Save_M_preTrain.py > /mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/Save_M_preTrain_lr=1e-5_epoch=3000.log 2>&1 &
'''