...
for epoch in range(EPOCH):
    ...
    if test_acc > best_acc:
        best_acc = test_acc
        if best_acc > 0.7 and best_acc < 0.72:
            np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_absorb_M_roc.txt'.format(best_acc),
                       roc_data1[:-1], delimiter=' ', fmt='%s')
            np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_absorb_M_roc2.txt'.format(best_acc),
                       roc_data1, delimiter=' ', fmt='%s')
            np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_absorb_M_prc.txt'.format(best_acc),
                       prc_data1[:-1], delimiter=' ', fmt='%s')
            np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc{}_absorb_M_prc2.txt'.format(best_acc),
                       prc_data1, delimiter=' ', fmt='%s')
            print('I\'ve finished my work!')
        # torch.save({"best_acc": best_acc,"metric":metric1, "model": net.state_dict()}, f'./{num_model}.pl')
        print(f"best_acc: {best_acc},metric:{metric1}")

'''
nohup python Absorb_M.py > /mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/absorb_M_lr=1e-4_epoch=300.log 2>&1 &

nohup python Absorb_M.py > /mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/absorb_M_lr=1e-4_epoch=300_nofreeze.log 2>&1 &
'''