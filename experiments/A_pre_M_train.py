...
for num_model in range(10):
    ...
    model = torch.load(
        '/mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/acc0.9432989690721649_model_Alter_preNew.pkl')  # 加载整个神经网络的模型结构以及参数
    net = model.to(device)
    lr = config.lr
    ...
    for epoch in range(EPOCH):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            # if best_acc > 0.78 and flag == 0:
            #     torch.save(net, '/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain.pkl'.format(best_acc))     # 保存整个神经网络的模型结构以及参数
            #     np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain_roc.txt'.format(best_acc), roc_data1[:-1], delimiter=' ', fmt = '%s')
            #     np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain_roc2.txt'.format(best_acc), roc_data1, delimiter=' ', fmt='%s')
            #     np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain_prc.txt'.format(best_acc), prc_data1[:-1], delimiter=' ', fmt = '%s')
            #     np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain_prc2.txt'.format(best_acc), prc_data1, delimiter=' ', fmt='%s')
            #     flag = 1
            #     # print('roc_data:\n', roc_data1)
            #     # print('\nprc_data\n', prc_data1)
            # # torch.save({"best_acc": best_acc,"metric":metric1, "model": net.state_dict()}, f'./{num_model}.pl')
            # if best_acc >= 0.94:
            #     torch.save(net, '/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain.pkl'.format(
            #         best_acc))  # 保存整个神经网络的模型结构以及参数
            #     np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain_roc.txt'.format(
            #         best_acc), roc_data1[:-1], delimiter=' ', fmt='%s')
            #     np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain_roc2.txt'.format(
            #         best_acc), roc_data1, delimiter=' ', fmt='%s')
            #     np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain_prc.txt'.format(
            #         best_acc), prc_data1[:-1], delimiter=' ', fmt='%s')
            #     np.savetxt('/mnt/sdb/home/yxt/CBC_and_DATA/tmp_experiment_results/acc{}_model_AedMain_prc2.txt'.format(
            #         best_acc), prc_data1, delimiter=' ', fmt='%s')


            print(f"best_acc: {best_acc},metric:{metric1}")

'''
nohup python A_pre_M_train.py > /mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/A_pre_M_train_lr=1e-5_epoch=3000.log 2>&1 &
nohup python A_pre_M_train.py > /mnt/sdb/home/yxt/CBC_and_DATA/experiment_results/A_pre_M_train2_lr=1e-5_epoch=3000.log 2>&1 &  
下面这个更加新，是使用了筛选数据的
'''